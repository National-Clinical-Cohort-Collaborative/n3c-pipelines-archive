CREATE TABLE `ri.foundry.main.dataset.75cb91ef-2993-41b9-9942-04fd6947dae1` AS
    ...
    covid_positive_A as (
        ...
        WHERE c.condition_concept_id IN (37311061) --concept_id of COVID positive in condition domain
    ...
    covid_positive_B as (
        ...
        WHERE m.measurement_concept_id IN (SELECT DISTINCT concept_id 
            ...
            WHERE concept_set_name IN ('ATLAS SARS-CoV-2 rt-PCR and AG', 'Atlas #818 [N3C] CovidAntibody retry') AND is_most_recent_version = true) 
            AND m.value_as_concept_id IN (4126681, 9191, 36032716, 45884084, 36715206, 45877985, 45878745, 45881802)
...
--measurement concepts from concept sets ATLAS SARS-CoV-2 rt-PCR and AG, Atlas #818 [N3C] CovidAntibody retry
    ...
    inpatient_admitted as (
        ...
        FROM `ri.foundry.main.dataset.749e3f71-9b0e-4896-af8b-91aefe71b308` v
        INNER JOIN `ri.foundry.main.dataset.456df227-b15d-4356-9df6-b113e90eb239` p
    ...
    covid_inpatient_death_cnt as (
        SELECT data_partner_id, COUNT(*) AS num_covid_inpatient_death_by_site
        FROM dup_elim
        GROUP BY data_partner_id
    )

    SELECT i.data_partner_id,  COALESCE(c.num_covid_inpatient_death_by_site, 0) as num_cov_inpat_death, i.total, c.num_covid_inpatient_death_by_site, ROUND((c.num_covid_inpatient_death_by_site / i.total * 100), 3) as inpatient_death_percentage
    FROM covid_inpatient_death_cnt c
    RIGHT JOIN inpatient_cnt i
    ON c.data_partner_id = i.data_partner_id