CREATE TABLE `/UNITE/[RP-4A9E27] DI&H - Data Quality/DQD_Tests/conditions/percent_covid_visits_by_type` AS

-- OF ALL COVID VISITS AT A SITE, WHAT IS THE BREAKDOWN OF VISIT TYPE


-- SELECT visits associated with a COVID diagnosis (UO7.1)
WITH  A AS
(
...
(
    SELECT  data_partner_id,
            COUNT(*) AS num_covid_visits_by_site
    FROM A
...
(
    SELECT  data_partner_id,
            visit_concept_name,
            COUNT(*) AS num_covid_visits_by_site_and_type
    FROM A
...
SELECT  B.data_partner_id,
        ...
        ROUND(C.num_covid_visits_by_site_and_type / B.num_covid_visits_by_site, 3) * 100 AS percent_covid_visits
