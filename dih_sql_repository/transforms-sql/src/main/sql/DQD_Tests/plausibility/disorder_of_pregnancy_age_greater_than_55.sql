CREATE TABLE `/UNITE/[RP-4A9E27] DI&H - Data Quality/DQD_Tests/plausibility/disorder_of_pregnancy_age_greater_than_55` AS
    
WITH I AS (
SELECT DISTINCT person_id, MAX(condition_start_date) as condition_start_date
FROM `/UNITE/LDS/clean/condition_occurrence`
WHERE condition_concept_id = 439658
GROUP BY person_id
),
...
FROM J
WHERE (year(condition_start_date) - year_of_birth) > 55
)
