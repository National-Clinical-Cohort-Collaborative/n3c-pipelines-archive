CREATE TABLE `/UNITE/[RP-4A9E27] DI&H - Data Quality/DQD_Tests/concept_mapping/concept_condition_domain` AS
    SELECT * FROM 
    (
        SELECT 
              d.person_id
            , d.condition_occurrence_id as id
            , d.condition_source_value as source_value
            , d.condition_concept_id as reported_concept_id
            , collect_list(d.concept_id_2) OVER (PARTITION BY d.condition_occurrence_id) as mapped_standard_concept_ids 
            , d.data_partner_id
            , 'Condition' as domain
        FROM
        (
            SELECT c.*, rel.concept_id_2 FROM
                (
                    ...
                    FROM `/UNITE/LDS/clean/condition_occurrence` dom
                        ...
                        ON dom.condition_source_value = concept.concept_code
