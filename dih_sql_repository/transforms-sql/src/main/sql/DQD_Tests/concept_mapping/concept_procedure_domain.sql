CREATE TABLE `/UNITE/[RP-4A9E27] DI&H - Data Quality/DQD_Tests/concept_mapping/concept_procedure_domain` TBLPROPERTIES (foundry_transform_profiles = 'EXECUTOR_MEMORY_MEDIUM') AS
    SELECT * FROM 
    (
        SELECT 
              d.person_id
            , d.procedure_occurrence_id as id
            , d.procedure_source_value as source_value
            , d.procedure_concept_id as reported_concept_id
            , collect_list(d.concept_id_2) OVER (PARTITION BY d.procedure_occurrence_id) as mapped_standard_concept_ids 
            , d.data_partner_id
            , 'Procedure' as domain
        FROM
        (
