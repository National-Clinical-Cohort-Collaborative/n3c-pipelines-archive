CREATE TABLE `/UNITE/[RP-4A9E27] DI&H - Data Quality/DQD_Tests/visits/visit_U09.9_percent_test` AS

WITH A AS
(
    SELECT COUNT(c_person_id) AS visits_U09_9
    FROM
    (
        SELECT c_person_id
        FROM
        (
            ...
            WHERE condition_concept_id = 705076
        )
        INNER JOIN 
        (
