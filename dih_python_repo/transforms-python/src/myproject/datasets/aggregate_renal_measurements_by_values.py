...
@transform_df(
    Output("/UNITE/[RP-4A9E27] DI&H - Data Quality/datasets/aggregate_renal_measurements_by_values"),
...
def grab_all_measurement_counts(persons, measurements, concept_sets):
    ...
    df_feature = df_feature.withColumn("value_as_number_exist", F.when(F.col("value_as_number").isNull(), F.lit("No_value")).otherwise(F.lit("value_exist")))

    # count by site and join to the df_join
    # Assuming domain_uniq_id is the correct column name for unique IDs
    df_final = df_feature.groupBy("data_partner_id", "Test", "value_as_concept_id", "value_as_concept_name", "value_as_number_exist").agg((F.countDistinct("measurement_id").cast("integer").alias("measurment_counts")))
    return df_final
