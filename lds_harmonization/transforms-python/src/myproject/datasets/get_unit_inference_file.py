...
@transform_df(
    Output("ri.foundry.main.dataset.a52b0501-6e17-4f33-8866-a94369301590"),
    inferred_unit_calc=Input("ri.foundry.main.dataset.c08ef024-1bfa-4e25-88ee-762dcb689d41"),
    canonicals_table=Input("ri.foundry.main.dataset.09b4a60a-3da4-4754-8a7e-0b874e2a6f2b")
...
def my_compute_function(inferred_unit_calc, canonicals_table):
    ...
    def array_median(x):
        medvals = [np.median(x[n].astype(float)) if ((len(x[n]) > 10)) else 0 for n in range(len(x))]
    ...
    def array_MADM_over_median(x):
        MADM_over_median = [stats.median_abs_deviation(n, axis=0, center=np.median, scale=1) /
                            np.median(n.astype(float))
                            for n in x]
