CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/04 - domain mapping/visit_occurrence` AS
...
with de_duplication as (
    ...
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/03 - prepared/encounter` encounter
    GROUP BY 
        patient_id,
        encounter_id,
        encounter_type,
...
SELECT
    ...
    FROM (
        ...
        FROM (
            ...
            FROM (
                select * , md5(site_encounter_id) as hashed_id
                ...
                -- add columns to ensure hashed_id uniqueness
                , md5(concat_ws(';',
                    ...
                    , COALESCE( cast( visit_start_date as string),'')
                    , COALESCE( cast(visit_type_concept_id as string),'')
                 )) as hashed_id
                FROM procedure
            )
        )
