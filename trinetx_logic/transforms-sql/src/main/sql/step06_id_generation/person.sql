CREATE TABLE `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/06 - id generation/person` AS

WITH join_conflict_id AS (
    SELECT 
        m.*
        , COALESCE(lookup.collision_bits, 0) as id_index
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/04 - domain mapping/person` m
    LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/05 - pkey collision lookup tables/person` lookup
...
pat_with_loc_id AS (
    SELECT 
        pat.site_patient_id,
        loc.*
    -- FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/03 - prepared/patient` pat
    -- use person from 04 - domain mapping? include postal code in step4 to person to prepare for location join in step6
    FROM `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/04 - domain mapping/person` pat
    LEFT JOIN `/UNITE/Data Ingestion & OMOP Mapping/Source Data Model: TriNetX/raw_trinetx/Site 102/transform/06 - id generation/location` loc
