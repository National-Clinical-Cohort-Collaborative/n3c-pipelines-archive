CREATE TABLE `ri.foundry.main.dataset.21a72f8d-4c45-47aa-9b93-652bd6feab5c` 
...
-- 		, data_partner_id
-- 		, payload
-- 		FROM `ri.foundry.main.dataset.311da417-1047-4c72-905e-9e2ec300a4fe`
...
-- 		  *
-- 		, md5(concat_ws( 
-- 			COALESCE(CAST(site_drug_era_id as string), '')
-- 		,   COALESCE( drug_concept_id, '')
-- 		,   COALESCE( CAST(drug_era_start_date AS string), '')
-- 		,   COALESCE( CAST( drug_era_end_date as string), '')
-- 		,   COALESCE( CAST( drug_exposure_count AS string), '') 
-- 		)) as hashed_id
-- 	FROM final_table
-- ) 
---- drug exposure has changed by remapping the concepts from the original dataset, re-run the drug_era code
