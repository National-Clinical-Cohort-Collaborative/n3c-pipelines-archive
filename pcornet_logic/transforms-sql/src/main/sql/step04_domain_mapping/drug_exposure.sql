CREATE TABLE `ri.foundry.main.dataset.0164a55d-e593-4508-b2b5-7cd9bb2f2bbe`
...
med_admin as (
    ...
    FROM `ri.foundry.main.dataset.4b6a2c8d-b352-4574-9861-3bfc764b0900` m
        ...
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` r 
...
prescribing as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.97e347ba-1a02-492f-8443-f0859a898389` pr
            ...
            LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` mx 
...
immunization as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.858a2339-3e51-4954-ba88-149584aab79d` im
        INNER JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
        ...
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` xw3
...
dispense as (
    SELECT 
    --- add your drug exposure mapping data from pcornet  data source 
        patid AS site_patid,
        CAST(xw.target_concept_id as int) as drug_concept_id,
        CAST(d.dispense_date as date) as drug_exposure_start_date, 
        CAST(d.dispense_date as timestamp) as drug_exposure_start_datetime,
        CAST(d.dispense_date as date) as drug_exposure_end_date, 
        CAST(d.dispense_date as timestamp) as drug_exposure_end_datetime, 
        CAST(null as date) as verbatim_end_date,
        32825 AS drug_type_concept_id,
        CAST(null as string) AS stop_reason,
        CAST(null as int) AS refills,
        CAST(d.dispense_amt as float) AS quantity,
        CAST(d.dispense_sup as int) AS days_supply,
        CAST(null AS string) AS sig,
        CAST(xw3.TARGET_CONCEPT_ID as int) AS route_concept_id,
        CAST(null AS string) AS lot_number,
        CAST(null AS long) AS provider_id, 
        CAST(null AS string) as site_encounterid,
        CAST(null as long) AS visit_detail_id,
        CAST(d.ndc as string)  as drug_source_value,
        CAST(xw.source_concept_id as int) as drug_source_concept_id,
        CAST(d.dispense_route as string) as route_source_value, 
        CAST(d.dispense_dose_disp_unit AS string) as dose_unit_source_value, 
        'DISPENSING' as domain_source,
        d.dispensingid as site_pkey,
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.5f3a9356-809c-465d-8725-f52dd918bd3c` d
        INNER JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
            ON trim(d.ndc) = trim(xw.src_code)
            AND xw.CDM_TBL = 'DISPENSING' 
            AND xw.target_domain_id = 'Drug'
            AND xw.src_code_type = 'NDC'
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` xw2 
            ON upper(trim(d.dispense_source)) = upper(trim(xw2.SRC_CODE)) 
            AND xw2.CDM_TBL = 'DISPENSING' 
            AND xw2.CDM_TBL_COLUMN_NAME = 'DISPENSE_SOURCE'  
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` xw3
            ON upper(trim(d.dispense_route)) = upper(trim(xw3.SRC_CODE)) 
            AND xw3.CDM_TBL = 'DISPENSING' 
            AND xw3.CDM_TBL_COLUMN_NAME = 'DISPENSE_ROUTE'  
