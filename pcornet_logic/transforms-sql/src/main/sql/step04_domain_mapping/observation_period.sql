CREATE TABLE `ri.foundry.main.dataset.9b7f68eb-3564-4cb3-b019-659f7e49c5af` AS
...
with obs_period AS (
	...
	FROM (
		...
		FROM `ri.foundry.main.dataset.7535361b-408a-40c4-8b34-3e782fc6c8c3`
		...
		UNION 
            ...
            FROM `ri.foundry.main.dataset.0164a55d-e593-4508-b2b5-7cd9bb2f2bbe`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
            SELECT 
				  site_patid
            ...
            FROM `ri.foundry.main.dataset.9eaa3af1-9d8a-4186-8291-b0e45a1fb3b3`
	        GROUP BY site_patid, data_partner_id, payload
	    UNION
            SELECT 
				  site_patid
            ...
            FROM `ri.foundry.main.dataset.0a7cc5a4-8681-4f2a-8b3b-956fd159352c`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
            SELECT 
				  site_patid
            ...
            FROM `ri.foundry.main.dataset.93dd35f5-b61d-4dac-ac0b-e5561310af58`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
		    SELECT 
				  site_patid
			...
			FROM `ri.foundry.main.dataset.f2557c45-d0f5-42ae-a8d7-43301380be00`
		    GROUP BY site_patid, data_partner_id, payload
		UNION 
			SELECT 
				  site_patid
		...
		UNION 
			...
			FROM `ri.foundry.main.dataset.071a4b37-c925-4738-bc46-d8af8676d885`
			GROUP BY site_patid, data_partner_id, payload
	) t
