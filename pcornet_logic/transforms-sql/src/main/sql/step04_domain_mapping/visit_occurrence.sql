CREATE TABLE `ri.foundry.main.dataset.419a0f18-8aa3-4459-9df8-c5b8b43f09b4` AS
...
-- encounter
    SELECT distinct
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.710e7df0-575a-4b76-b799-bc0488f10e68` enc
        LEFT JOIN `ri.foundry.main.dataset.6f475189-e4bc-4cac-8341-5dbaf03fbbd5` vx
        ...
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` disp 
...
--   these obs_gen records already have encounterids. Make sure to hash a unique site id by adding string prefix.
    SELECT distinct
        ...
        cast (32831 as int) AS visit_type_concept_id,
        CAST(null AS long) as provider_id,
        CAST(null AS long) as care_site_id,
    ...
    WHERE obg.obsgen_type = 'PC_COVID'
        ...
        ---when enc.enc_type in ('OT', 'OS', 'OA') then 38000269
        ---else 0 end AS VISIT_TYPE_CONCEPT_ID,  --check with SMEs
        case when ( vsrc.TARGET_CONCEPT_ID != 0 and vsrc.TARGET_CONCEPT_ID is not null) then cast( vsrc.TARGET_CONCEPT_ID as int)
            else cast (32035 as int)
    ...
    FROM `ri.foundry.main.dataset.da113963-4e92-42a2-9477-2591001cfcc1` pr
       /*
        LEFT JOIN `ri.foundry.main.dataset.6f475189-e4bc-4cac-8341-5dbaf03fbbd5` vx
            ON vx.CDM_TBL = 'PROCEDURES' 
            AND vx.CDM_NAME = 'PCORnet' 
            AND trim(vx.SRC_VISIT_TYPE) = trim(pr.enc_type)
        */
