CREATE TABLE `ri.foundry.main.dataset.0a7cc5a4-8681-4f2a-8b3b-956fd159352c`
...
with condition as (
    SELECT DISTINCT
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.0c17b2c1-862d-4103-9c4d-25507bcc13e4` c
        JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
...
-- PAT_PREF_LANGUAGE_SPOKEN
    ...
    FROM `ri.foundry.main.dataset.ef9e40d3-9847-442c-98a6-80c6becfc6b9` demo
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` lang 
        ...
        LEFT JOIN (
            ...
            FROM `ri.foundry.main.dataset.710e7df0-575a-4b76-b799-bc0488f10e68` 
            GROUP BY patid
            ) ee 
...
-- GENDER_IDENTITY
    SELECT DISTINCT
        ...
        , payload
    FROM `ri.foundry.main.dataset.ef9e40d3-9847-442c-98a6-80c6becfc6b9` demo     
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` gx 
...
-- SEXUAL_ORIENTATION
    SELECT DISTINCT
        ...
        , payload
    FROM `ri.foundry.main.dataset.ef9e40d3-9847-442c-98a6-80c6becfc6b9` demo   
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` sx 
...
diagnosis as (
    ...
    FROM `ri.foundry.main.dataset.eafe63b1-42db-4f3f-ac2f-0b96f91c5260` d
        ...
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` xw2 
...
lab_result_cm as (
    SELECT
        ...
        , data_partner_id
        , payload
        FROM `ri.foundry.main.dataset.634e56ef-35da-4cbb-9290-cf8950708c92` lab
        ...
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` u 
...
obs_clin as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.3dacc398-2418-4bd9-9dac-4a67a1e000ad` obs
        INNER JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
...
obs_gen as ( 
    ...
    CAST(null as string) AS qualifier_source_value,
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.b52c4215-d873-4931-a8a8-455282ec9a06` obsg
...
obsgen_sdoh as (
   ...
   --- COALESCE(xw_map1.TARGET_CONCEPT_ID, xw_map2.TARGET_CONCEPT_ID) AS value_as_concept_id,  --- use xw2 or xw3, this would be the concept id of the categorical data on obsgen_result_text
    ...
    LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` obsgensrc
            ON obs.obsgen_source = obsgensrc.SRC_CODE
            AND obsgensrc.CDM_TBL = 'OBS_GEN'
            AND obsgensrc.CDM_TBL_COLUMN_NAME = 'OBSGEN_SOURCE'
    LEFT JOIN `ri.foundry.main.dataset.9c88070e-c967-4662-a45b-73bd72b579b1`  answercode
   ...
   --- COALESCE(xw_map1.TARGET_CONCEPT_ID, xw_map2.TARGET_CONCEPT_ID) AS value_as_concept_id,  --- use xw2 or xw3, this would be the concept id of the categorical data on obsgen_result_text
    ...
    LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` obsgensrc
            ...
            AND obsgensrc.CDM_TBL = 'OBS_GEN'
            AND obsgensrc.CDM_TBL_COLUMN_NAME = 'OBSGEN_SOURCE'
    LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` modifier
    ...
    LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` categorical
...
pro_cm as (
    SELECT
        patid AS site_patid,
        CAST(xw.target_concept_id as int) AS observation_concept_id,
        CAST(pro.pro_date as date) AS observation_date,
        CAST(pro.PRO_DATETIME as timestamp) AS observation_datetime, 
        CAST(prosrc.TARGET_CONCEPT_ID as int) AS observation_type_concept_id,
        CAST(pro.pro_response_num as float) AS value_as_number,
        CAST(pro.pro_response_text as string) AS value_as_string,
        ...
        encounterid as site_encounterid,
        CAST(null as long) AS visit_detail_id,
        CAST(pro.pro_item_loinc as string) AS observation_source_value,
        CAST(xw.target_concept_id as int) AS observation_source_concept_id,
        CAST(null as string) AS unit_source_value, -- lookup unit_concept_code
        CAST(null as string) AS qualifier_source_value,
    ...
    FROM `ri.foundry.main.dataset.7769b85a-98eb-473b-a414-843d7c7ffde3` pro
        INNER JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
            ON pro.pro_item_loinc = xw.src_code
            AND xw.CDM_TBL = 'PRO_CM' AND xw.target_domain_id = 'Observation' 
        --INNER JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
            --ON pro.pro_measure_loinc = xw.src_code
            --AND xw.CDM_TBL = 'PRO_CM' AND xw.target_domain_id = 'Observation' 
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` prosrc
            ON trim(pro.pro_source) = trim(prosrc.SRC_CODE)
            AND prosrc.CDM_TBL = 'PRO_CM'
            AND prosrc.CDM_TBL_COLUMN_NAME = 'PRO_SOURCE'
),
...
procedures as (
    SELECT
        ...
        payload
    FROM
        `ri.foundry.main.dataset.da113963-4e92-42a2-9477-2591001cfcc1` pr
        JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
...
-- Smoking
    SELECT
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.fbe7f6f9-c4de-4562-8f71-733706fbf168` v
        INNER JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` s 
            ON s.CDM_TBL = 'VITAL'
            AND s.CDM_TBL_COLUMN_NAME = 'SMOKING'
            AND s.SRC_CODE = v.smoking
        LEFT JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` vt 
            ON vt.CDM_TBL = 'VITAL'
...
-- Tobacco
    SELECT
        ...
        payload
    FROM `ri.foundry.main.dataset.fbe7f6f9-c4de-4562-8f71-733706fbf168` v
        JOIN `ri.foundry.main.dataset.9cd6e457-ddf3-416a-9bad-ffc36b30fcf3` s 
            ON s.CDM_TBL = 'VITAL'
...
encounter as (
    SELECT
        ...
        data_partner_id,
        payload
    FROM `ri.foundry.main.dataset.710e7df0-575a-4b76-b799-bc0488f10e68` d
...
all_other_tables AS (
    ...
    FROM (
        ...
        FROM `ri.foundry.main.dataset.5f3a9356-809c-465d-8725-f52dd918bd3c` d
        INNER JOIN `ri.foundry.main.dataset.05c59431-1c54-4ef8-a475-b3772d8e830f` xw 
