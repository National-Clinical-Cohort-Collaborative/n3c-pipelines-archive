...
@transform_df(
    Output("ri.foundry.main.dataset.1a2d0d02-c79a-4104-9cba-c0acf5e63aff"),
    concept_df=Input("ri.foundry.main.dataset.5cb3c4a3-327a-47bf-a8bf-daf0cafe6772"),
    **inputs,
)
def my_compute_function(concept_df, **inputs):
    final_df = None
    concept_df = concept_df.select("concept_id", "concept_class_id")
    for domain, df in inputs.items():
        for col in df.columns:
            if col.endswith("_concept_id"):
                # Create a df with all unique concept_class_ids for the concept_ids in this column 
                this_col_df = df.join(
                    concept_df,
                    df[col] == concept_df["concept_id"],
                    "left_outer"
                ).withColumn("domain", F.lit(domain)).withColumn("column", F.lit(col))
                this_col_df = this_col_df.select("domain", "column", "concept_class_id").distinct()
                if final_df:
