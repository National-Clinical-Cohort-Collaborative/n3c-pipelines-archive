...
from pyspark.sql.functions import col, concat_ws, coalesce, upper
from pyspark.sql import functions as F
from act.anchor import path


@transform(
    processed=Output(path.transform + '03 - prepared/observation_fact'),
    my_input=Input(path.transform + '02 - clean/observation_fact'),
    site_id_df=Input(path.site_id),
    local_code_map=Input(path.metadata + 'act_local_code_map'),
    vocab_id_map=Input(path.mapping),
    vocab_id_map_overrides=Input(path.mapping_overrides)
)
def compute_function(my_input, site_id_df, local_code_map, vocab_id_map, vocab_id_map_overrides, processed):
    ...
    processed_df = processed_df.join(
        local_code_map,
        (upper(processed_df["concept_cd"]) == upper(local_code_map["local_concept_cd"])),
        "left"
