CREATE TABLE `ri.foundry.main.dataset.789b02e5-4c62-4c54-b1e9-d72bcba3ea92` AS
...
with visit_detail as (
    SELECT DISTINCT
        ...
        -- add more data values here to make it unique
        CAST(site_comparison_key || '|' || vd.start_date || '|' || COALESCE( vd.end_date, observation_blob ) as string) || '|' || vd.encounter_num  as site_visit_detail_id
        ...
        , CAST(vd.end_date as timestamp) AS visit_detail_end_datetime
        , 32817 AS visit_detail_type_concept_id
        , CAST(vd.provider_id as int) AS site_provider_id
        ...
        , vd.data_partner_id
        , vd.payload
    FROM `ri.foundry.main.dataset.b45684f1-ba10-4a41-a49b-34a80dee15c4` vd
    --encounter num for adt data should be a valid visit in the visit_dimension
    INNER JOIN  `ri.foundry.main.dataset.8466536f-cdf7-4818-8197-24a315bc5b52` enc
