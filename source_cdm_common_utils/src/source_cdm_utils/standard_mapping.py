"""
This file contains the helper function that corrects the nons tandard concepts
-- Stephanie Hong , July 2023 
"""
...
# @transform_df(
#     Output("/UNITE/Data Ingestion & OMOP Mapping/Shared Logic/src/source_cdm_utils/standard_mapping"),
...
def perform_standard_mapping(df, concept_df, concept_relationship_df, concept_col, incoming_domain_id):
    if "domain_id" in df.columns:
        df = df.withColumnRenamed("domain_id", "incoming_domain_id")

        # when the incoming domain id in the dataset is null then use the incoming domain id parameter passed in so that
        # we know that the where the data originated from 
        df = df.withColumn("incoming_domain_id", F.when(
            F.col("incoming_domain_id").isNull(), F.lit(incoming_domain_id)
            ).otherwise(F.col("incoming_domain_id"))
        )
