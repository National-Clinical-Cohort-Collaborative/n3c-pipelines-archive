    ...
    for col_name, col_type in schema_dict.items():
        if col_name in input_cols:
            if col_type == T.DateType() or col_type == T.TimestampType():
                ...
                # Handle site 861's format
                df = df.withColumn(
                    col_name,
                    F.when(
                        df[col_name].rlike("^\\d{4}\\.\\d{2}\\.\\d{2} \\d{2}:\\d{2}:\\d{2}$"),
                        F.from_unixtime(F.unix_timestamp(df[col_name], format='yyyy.MM.dd HH:mm:ss'))
                    ).otherwise(df[col_name])
                )
                ...
                df = df.withColumn(
                    col_name,
                    F.when(
                        df[col_name].rlike("^\\d{4}\.\\d{2}\.\\d{2}$"),
                        F.from_unixtime(F.unix_timestamp(df[col_name], format='yyyy.MM.dd'))
                    ).otherwise(df[col_name])
                )
...
omop_complete = {
    ...
    "health_system": {
        "HEALTH_SYSTEM_ID": T.LongType(),
        "COMPENDIUM_YEAR": T.IntegerType(),
        "HEALTH_SYS_ID": T.StringType(),
        "PLACE_OF_SERVICE_CONCEPT_ID": T.IntegerType(),
        "LOCATION_ID": T.LongType(),
        "CARE_SITE_SOURCE_VALUE": T.StringType(),
        "PLACE_OF_SERVICE_SOURCE_VALUE": T.StringType(),
        "TEACHING_INTENSITY": T.IntegerType(),
        "TEACHING_INTENSITY_CATEGORY": T.StringType(),
        "MULTISTATE": T.IntegerType(),
        "MULTISTATE_CATEGORY": T.StringType(),
        "CARE_SITE_SIZE": T.IntegerType(),
        "CARE_SITE_SIZE_CATEGORY": T.StringType(),
        "HEALTH_SYSTEM_HASH_ID": T.StringType(),
    },
    "location": {
        "LOCATION_ID": T.LongType(),
